...
async def read_llm(ctx, link: str):
    ...
    if is_hyv:
        ...
        prompt = (
            ...
            "If the event has region-specific times (Asia, America, Europe), convert 'server time' to each region's server time using these timezones: "
            "Asia: Asia/Shanghai (UTC+8), America: America/New_York (UTC-5), Europe: Europe/Berlin (UTC+1). "
            "Extract each region's start and end time as UNIX timestamps (seconds since epoch, UTC). "
            "If the start time is described as 'Version Start', extract the version number and indicate it as 'Version Start <version>'. "
            "Reply in this exact format (one line per field, no extra text, no quotes, only integer UNIX timestamps or 'None' or 'Version Start <version>'):\n"
            "The Profile should be one of these: [HSR, ZZZ] (It is the abbreviation of the twitter handle)\n"
            "The Category should be one of these single words: [Banner, Event, Maintenance] (Do not use phrases. Anything that is a Headhunting/Gacha/Warp/Channels or has the word Limited Time is considered a Banner)\n"
            "If the Category is Banner, the Title should include the name of the Character in the Banner\n"
            "Title: <title>\n"
            "Category: <category>\n"
            "Profile: <profile>\n"
            "Asia Start: <UNIX timestamp or Version Start <version> or None>\n"
            "Asia End: <UNIX timestamp or None>\n"
            "America Start: <UNIX timestamp or Version Start <version> or None>\n"
            "America End: <UNIX timestamp or None>\n"
            "Europe Start: <UNIX timestamp or Version Start <version> or None>\n"
            "Europe End: <UNIX timestamp or None>\n"
            "Timezone: <timezone or None>\n"
            "If any field is missing, write None for that field.\n"
            "Example:\n"
            "Title: Version 3.1 Warp\n"
            "Category: Banner\n"
            "Profile: HSR\n"
            "Asia Start: 1746330000\n"
            "Asia End: 1747420740\n"
            "America Start: 1746308400\n"
            "America End: 1747400040\n"
            "Europe Start: 1746322800\n"
            "Europe End: 1747414440\n"
            "Timezone: UTC+8\n"
            f"Text:\n{tweet_text}"
        )
        llm_response = await run_phi2_inference(prompt)
    ...
    prompt = (
        ...
        "The Category should be one of these single words: [Banner, Event, Maintenance, Offer] (Do not use phrases. Anything that is a Headhunting/Gacha/Warp/Channels or has the word Limited Time is considered a Banner)\n"
        "If the Category is Banner, the Title should include the name of the Character in the Banner\n"
        "Title: <title>\n"
        "Category: <category>\n"
        "Profile: <profile>\n"
