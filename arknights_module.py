...
async def classify_and_extract_ak_event(tweet_text, tweet_image):
    """
    COMBINED LLM call: Classifies tweet AND extracts event data in one prompt.
    Returns dict with:
    - classification: "Event" or "Filler"
    - title, category, start, end, image (only if classification is "Event")
    
    This reduces LLM API calls by 50% compared to separate classification + extraction.
    """
    ak_logger.info("=== [classify_and_extract_ak_event] COMBINED CLASSIFICATION + EXTRACTION ===")
    ...
    prompt = (
        ...
        "- Look for sections labeled 'EVENT STAGE' or the main event section (usually labeled '01')\n"
        "- Find the 'DURATION' field directly under 'EVENT STAGE'\n"
        "- This will show: 'Month Day, Year, HH:MM - Month Day, Year, HH:MM (UTC-7)'\n"
        "- **IGNORE** other duration fields like:\n"
        "  - 'LIMITED-TIME PACKS AVAILABLE' dates\n"
        "  - '[PATH OF TRIALS]' dates\n"
        "  - Subsection dates that differ from the main event\n"
        "- Extract ONLY the main 'EVENT STAGE DURATION' or equivalent top-level event duration\n"
        "\n"
        "**CLASSIFICATION RULES:**\n"
        "\n"
        "Classify as 'Event' ONLY if ALL of these are true:\n"
        "1. Announces a new banner/headhunting (with ★ stars showing operator rarities), in-game event with stages, or server maintenance\n"
        "2. Contains BOTH a start date AND end date (or time range)\n"
        "3. Is NOT an outfit/skin announcement (even if it has dates)\n"
        "4. Is NOT a trailer, PV (Promotional Video), or animation preview\n"
        "5. Is NOT a character advertisement without rate-up stars (★)\n"
        "\n"
        "Classify as 'Filler' if ANY of these are true:\n"
        "- Outfit/skin announcements (look for 【brackets】, 'Tailor-Provided', 'outfit', 'skin', 'FOR FREE')\n"
        ...
        "- Trailers, PVs, Animation PVs (contains 'Official Trailer', 'PV', 'Animation PV', 'HD version:', YouTube links)\n"
        "- Character advertisements WITHOUT star ratings (★★★★★★, ★★★★★, etc.)\n"
        "- New operator announcements that are just promotional content (look for 【New Operators】 WITHOUT rate-up stars)\n"
        "\n"
        "IMPORTANT for Banners:\n"
        "- Banners MUST show star ratings (★★★★★★, ★★★★★) to indicate rate-up operators\n"
        "- If tweet says 'Operators will appear at a higher rate' or 'rated up' AND shows stars, it's an Event\n"
        "- If tweet says 【New Operators】 but has NO stars shown, it's just a character ad → Filler\n"
        "\n"
        "IMPORTANT for Event Announcements:\n"
        "- 'New Story Collection Event: [Name] will soon be live' with dates → Event\n"
        "- 'New Story Collection Event: [Name] will be live' in a trailer/PV with YouTube link → Filler\n"
        "- Check for phrases like 'Official Trailer', 'Animation PV', 'HD version:' → Always Filler\n"
        "\n"
        "**OUTPUT FORMAT:**\n"
        "\n"
        "If classification is 'Filler', respond with ONLY:\n"
        "```\n"
        "Classification: Filler\n"
        "```\n"
        "\n"
        "If classification is 'Event', respond with:\n"
        "```\n"
        "Classification: Event\n"
        "Title: <event title>\n"
        "Category: <Banner|Event|Maintenance>\n"
        "Start: <Month Day, Year, HH:MM (UTC-7)>\n"
        "End: <Month Day, Year, HH:MM (UTC-7)>\n"
        "```\n"
        "\n"
        "**DATE FORMAT:**\n"
        "\n"
        "Output dates in this EXACT format: 'Month Day, Year, HH:MM (UTC-7)'\n"
        "Examples:\n"
        "- November 20, 2025, 08:00 (UTC-7)\n"
        "- December 4, 2025, 03:59 (UTC-7)\n"
        "- September 16, 2025, 10:00 (UTC-7)\n"
        "\n"
        "CRITICAL:\n"
        "- ALWAYS include the YEAR (2025, not 2024)\n"
        "- ALWAYS include (UTC-7) at the end\n"
        "- Use full month names (November, not Nov)\n"
        "- Use 24-hour time format (08:00, not 8:00 AM)\n"
        "\n"
        "**EXTRACTION RULES (only for Events):**\n"
        "\n"
        "- Title:\n"
        "  - For banners: Include 6★ operator names (e.g., 'Mon3tr Banner', 'Exusiai & Siege Kernel Banner')\n"
        "  - For events: Use event name from tweet (e.g., 'Trials for Navigator #05', 'Act or Die')\n"
        "  - For maintenance: Use date + 'Maintenance' (e.g., 'September 16 Maintenance')\n"
        "\n"
        "- Category: Must be one of: Banner, Event, Maintenance\n"
        "  - 'Banner' if mentions: operator, headhunting, rate up, Limited\n"
        "  - 'Event' if mentions: episode, stages, trials, sidestory, new event\n"
        "  - 'Maintenance' if mentions: maintenance, server downtime\n"
        "\n"
        "- Start/End:\n"
        "  - Extract dates from the main EVENT STAGE DURATION field only\n"
        "  - Output in format: Month Day, Year, HH:MM (UTC-7)\n"
        "  - Ensure the YEAR is correct (look carefully at the image/tweet)\n"
        "\n"
        "**EXAMPLES:**\n"
        "\n"
        "Example 1 (Event from Image):\n"
        "Tweet: 'Trials for Navigator #05 is now open!'\n"
        "Image shows: EVENT STAGE DURATION: November 20, 2025, 08:00 - December 4, 2025, 03:59 (UTC-7)\n"
        "Response:\n"
        "```\n"
        "Classification: Event\n"
        "Title: Trials for Navigator #05\n"
        "Category: Event\n"
        "Start: November 20, 2025, 08:00 (UTC-7)\n"
        "End: December 4, 2025, 03:59 (UTC-7)\n"
        "```\n"
        "\n"
        "Example 2 (Event with Text Dates):\n"
        ...
        "Title: Mon3tr Banner\n"
        "Category: Banner\n"
        "Start: September 16, 2025, 10:00 (UTC-7)\n"
        "End: September 30, 2025, 03:59 (UTC-7)\n"
        "```\n"
        "\n"
        ...
        "```\n"
        "Classification: Event\n"
        "Title: September 16 Maintenance\n"
        "Category: Maintenance\n"
        "Start: September 16, 2025, 10:00 (UTC-7)\n"
        "End: September 16, 2025, 10:10 (UTC-7)\n"
        "```\n"
        "\n"
        "Example 4 (Filler - Outfit):\n"
        ...
        "```\n"
        "\n"
        "Example 6 (Filler - Official Trailer/PV):\n"
        "Tweet: Arknights Official Trailer - Fantasy in the Mirage\n\n"
        "\"Is it an act of kindness to bring them into this tumultuous world, or an act of cruelty?\"\n\n"
        "New Story Collection Event: Fantasy in the Mirage will be live on December 11, 2025, 08:00 (UTC-7)!\n\n"
        "HD version: https://youtu.be/10YZp9QFN3Q\n\n"
        "#Arknights #Yostar\n"
        "Response:\n"
        "```\n"
        "Classification: Filler\n"
        "```\n"
        "Reason: Contains 'Official Trailer' and YouTube link. This is promotional content, not the actual event announcement.\n"
        "\n"
        "Example 7 (Filler - Animation PV):\n"
        "Tweet: Arknights Animation PV - Path of Life Rerun\n\n"
        "\"The reason our current joint front exists. It is precisely because the 'path' was charted.\"\n\n"
        "Rerun Side Story Event: Path of Life will be live on November 27, 2025, 08:00 (UTC-7)!\n\n"
        "HD version: https://youtu.be/5dbOHFv5tI0\n\n"
        "#Arknights #Yostar\n"
        "Response:\n"
        "```\n"
        "Classification: Filler\n"
        "```\n"
        "Reason: Contains 'Animation PV' and YouTube link. This is a video trailer, not the event announcement.\n"
        "\n"
        "Example 8 (Filler - Character Advertisement Without Stars):\n"
        "Tweet: 【New Operators】\n\n"
        "Operators Leizi the Thunderbringer and Record Keeper will be rated up in the Limited-time Headhunting: The Rolling Thunder between December 11, 2025, 08:00 – December 25, 2025, 03:59 (UTC-7)!\n"
        "Response:\n"
        "```\n"
        "Classification: Filler\n"
        "```\n"
        "Reason: Has 【New Operators】 but shows NO star ratings (★). This is a character advertisement, not the actual banner announcement.\n"
        "\n"
        "Example 9 (Event - Banner WITH Stars):\n"
        "Tweet: Dear Doctor,\n\n"
        "The following Operators will appear at a higher rate between December 11, 2025, 08:00 (UTC-7) - December 25, 2025, 03:59 (UTC-7).\n\n"
        "★★★★★★: Leizi the Thunderbringer\n"
        "★★★★★: Record Keeper/ Warmy\n"
        "Response:\n"
        "```\n"
        "Classification: Event\n"
        "Title: Leizi the Thunderbringer Banner\n"
        "Category: Banner\n"
        "Start: December 11, 2025, 08:00 (UTC-7)\n"
        "End: December 25, 2025, 03:59 (UTC-7)\n"
        "```\n"
        "Reason: Shows star ratings (★★★★★★, ★★★★★) indicating rate-up operators. This is the actual banner announcement.\n"
        "\n"
        "Example 10 (Event - Story Collection Event Announcement):\n"
        "Tweet: Dear Doctor,\n"
        "New Story Collection Event: Fantasy in the Mirage will soon be live on December 11, 2025, 08:00 (UTC-7), and some of the contents are available for a limited time. Please refer to the following notification for the event details.\n"
        "Response:\n"
        "```\n"
        "Classification: Event\n"
        ...
        "Category: Event\n"
        "Start: December 11, 2025, 08:00 (UTC-7)\n"
        "End: (extract from image or assume 2 weeks later)\n"
        "```\n"
        "Reason: This is the actual event announcement with a start date, NOT a trailer. No YouTube link or 'Official Trailer' text.\n"
        "\n"
    ...
    # Parse response - extract only the structured fields (ignore explanations)
    response_lines = response.split('\n')
    
    # Find all field lines (Classification:, Title:, Category:, Start:, End:)
    field_lines = []
    for line in response_lines:
        line_stripped = line.strip()
        if line_stripped and any(line_stripped.lower().startswith(f"{field}:") for field in ["classification", "title", "category", "start", "end"]):
    ...
    if not classification:
        ak_logger.warning("⚠️ No classification found in response, assuming Filler")
        return {"classification": "Filler"}
    
    # If Filler, return early
    if classification.lower() == "filler":
    ...
    ak_logger.info(f"Extracted: title={title}, category={category}, start={start}, end={end}")
    ...
    if not title:
        ...
        ak_logger.info(f"Title missing, using parse_title_ak: {title}")
    
    # Validate title
    invalid_titles = ["arknights_en", "arknights en", "@arknightsen", "arknights", "twitter.com/arknights"]
    if title and any(invalid in title.lower() for invalid in invalid_titles):
        ak_logger.error(f"❌ INVALID TITLE: '{title}' - Rejecting event")
        return {"classification": "Filler"}
    
    # Category fallback
    if not category:
        category = parse_category_ak(tweet_text)
        ak_logger.info(f"Category missing, using parse_category_ak: {category}")
    
    # Dates fallback - convert to UNIX if needed
    def to_unix(date_str):
        if not date_str:
            return None
        # If already looks like UNIX timestamp, return it
        if date_str.isdigit():
            return int(date_str)
        # Otherwise parse with dateparser
        import dateparser
    ...
    # If dates still missing, try parse_dates_ak
    if not start_unix or not end_unix:
        ak_logger.info("Dates missing or invalid, using parse_dates_ak fallback...")
        parsed_start, parsed_end = await parse_dates_ak(None, tweet_text)
        if not start_unix and parsed_start:
            start_unix = to_unix(parsed_start)
        if not end_unix and parsed_end:
            end_unix = to_unix(parsed_end)
    
    ak_logger.info(f"Final UNIX: start={start_unix}, end={end_unix}")
    
    # Use tweet_image directly (already extracted by twitter_handler)
...
async def ak_read(ctx, link: str):
    ...
    if result["classification"] == "Filler":
        await ctx.send("This tweet is not classified as an event (Filler content).")
        return
...
async def ak_read_test(ctx, link: str):
    ...
    # Use combined LLM call
    await ctx.send("Analyzing tweet with LLM (combined classification + extraction)...")
    result = await classify_and_extract_ak_event(tweet_text, tweet_image)
    
    await ctx.send(f"**Classification:** {result['classification']}")
    ...
    # Show extracted info
    await ctx.send(
        f"**This tweet is classified as an event!**\n"
        f"**Extracted Event Info:**\n"
        f"Title: {result['title']}\n"
        f"Category: {result['category']}\n"
        f"Start: <t:{result['start']}:F> (UNIX: {result['start']})\n"
        f"End: <t:{result['end']}:F> (UNIX: {result['end']})\n"
