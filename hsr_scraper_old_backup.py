...
async def init_prydwen_db():
    ...
    async with aiosqlite.connect(DB_PATH) as conn:
        ...
        # Migrate old regional columns to single columns if they exist
        ...
        # Migrate data from old regional columns to new single columns (use NA as primary)
        if 'na_start_date' in column_names and 'start_date' in column_names:
            ...
            await conn.execute('UPDATE events SET start_date = na_start_date WHERE start_date IS NULL')
            await conn.execute('UPDATE events SET end_date = na_end_date WHERE end_date IS NULL')
